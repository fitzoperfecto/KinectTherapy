...
namespace SWENG
{
    ...
    class AngleCriterion:Criterion
    {
        ...
        public override bool matchesCriterion(SkeletonStamp skeletonStamp)
        {
            ...
            Vector2 vector1 = new Vector2(joint0.Position.X - vertexJoint.Position.X, joint0.Position.Y - vertexJoint.Position.Y);
            // get vector vertex -> joint1
            Vector2 vector2 = new Vector2(joint1.Position.X - vertexJoint.Position.X, joint1.Position.Y - vertexJoint.Position.Y);

            // determine the angle from two vectors
            float computedAngle = (float)Math.Atan2(vector1.Y, vector1.X) - (float)Math.Atan2(vector2.Y, vector2.X);
